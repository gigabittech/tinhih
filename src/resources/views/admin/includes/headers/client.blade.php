            ...
            <ul class="header-nav align-items-center">
                <li class="nav-item">
                    <a class="dropdown-item" style="color: rgba(255, 255, 255, 0.6)"
                        href="{{ route('client.manage') }}">Profile Settings</a>
                </li>

            </ul>
        </div>
        <a class="header-brand d-md-none" href="dashboard.index">
                    ...
                    @if (auth()->user()->unreadNotifications()->count() > 0)
                        ...
                        @foreach (auth()->user()->unreadNotifications as $notification)
                            <p style=" color: rgba(255,255,255, 0.6);" class="ps-3 pb-0 my-2">
                                {{ auth()->user()->type == 'client' ? $notification->data['clientMessage'] : '' }}
                                {{-- {{ auth()->user()->type == 'client' ? $notification : '' }} --}}
                        ...
                        <a class="dropdown-item text-primary mt-1" href="{{ route('notification.markRead') }}">Mark all
                            as
                            read</a>
                    @else
            ...
            <li class="nav-item dropdown">
                <a class="nav-link py-0" data-coreui-toggle="dropdown" href="#" role="button"
                    ...
                    <div class="avatar avatar-md">
                        @if (isset($client->client_image))
                            <img class="avatar-img mt-3" src="{{ asset($client->client_image) }}"
                                alt="user@email.com" />
                        @else
                ...
                <div class="dropdown-menu dropdown-menu-end pt-0 mt-1 bg-dark">
                    ...
                    <a class="dropdown-item" style="color: rgba(255, 255, 255, 0.6);"
                        ...
                        <i class="fa-solid fa-file-pen me-2 text-warning" style="margin-top:9px;"></i>
                        Identifying Data</a>
